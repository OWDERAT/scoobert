...
async function doJb() {
  ...
  try {
    ...
    if (fn.setuid.invoke(0) === -1) {
      ...
      const kpatches_rsp = await fetch(
        `src/ps4/patches/${constants.KPATCH}`
      );

      const kpatches_buf =
        await kpatches_rsp.arrayBuffer();

      const kpatches_u8 =
        new Uint8Array(kpatches_buf);

      kernel_patches(kpatches_u8);

      const bin_rsp =
        await fetch("src/payload.bin");

      const bin_buf =
        await bin_rsp.arrayBuffer();

      const bin_u8 =
        new Uint8Array(bin_buf);

      load_bin(bin_u8);

      // SUCCESS POPUP
      showScoobertSuccess(
        "Scoobert has jailbroken your PS4 with GoldHen! Have fun Onichan :>"
      );
    }
